...
if __name__ == "__main__":  #TODO: Why use this sentence
    ...
    for seed in Seed_set:
        ...
        while True:  # TODO: What is the difference with for loop over clients
            ...
            Average_weights = Transfer.Average(client_weights)

            for n in range(CLIENTS):
                model.assign_weights(weights=Average_weights[n])
                model.model.train()
                ...
                Vector_update = (1 - 0.5 * (iter_num + 1)) * client_weights[n] + 0.5 * (iter_num + 1) * Vector_update

                Vector_update, _ = client_compressor[n].get_trans_bits_and_residual(w_tmp=Vector_update, w_residual=client_residual[n])
                client_weights[n] = (1 - 2 / (iter_num + 1)) * client_weights[n] + (2 / (iter_num + 1)) * Vector_update
    ...
    f = open('ECD_{}_{}.txt'.format(RATIO, ROUND_ITER), 'w')
